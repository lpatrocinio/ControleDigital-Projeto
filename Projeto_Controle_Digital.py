...
local = ['Franca', 'Franca', 'Ribeirão Preto', 'Ribeirão Preto', 'São Carlos', 'São Carlos', 'Tietê', 'Santo André', 'Terra Roxa', 'Vitória']
height = [996, 996, 531, 531, 854, 854, 482, 766, 498, 4]
latitude = [-20.5418, -20.5418, -21.1767,  -21.1767, -22.0154, -22.0154, -23.1123,-23.6666, -20.7895, -20.3222]
...
for i in range(10):
    ...
    print(f"omega: {omega * 30 / np.pi} RPM")
    _omega.append(omega * 30 / np.pi)

    # ideal torque
    Q = 0.5 * air_density * cq_br * np.pi * (radius ** 5) * (omega ** 2)
    Aq = r_est * Q / (kt * v_bat)
    # motor BEMF
    Q_bemf = kw * omega #V_set
    print(f"V_set: {Q_bemf}")
    _vset.append(Q_bemf)
    ...
    print(f"Rho: {air_density}")
    rho_lista.append((air_density))

print(f"Lista Rho: {rho_lista} \n")
print(f"Lista de Vset: {_vset} \n")
print(f"Lista de Velocidade : {_omega} \n")
